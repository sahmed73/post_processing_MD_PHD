...
"""
Author: Shihab Ahmed
Created on Fri Nov  1 15:13:58 2024
"""
...
result = {}
for sim in range(8):
    dirr=r"C:\Users\arup2\OneDrive - University of California Merced\Desktop\LAMMPS\borgstore\ARLOI\ARLOI-V1\10_ARLOI_NPT-high_NPT-cooling_NPT-low_SORIA\A0001\Production\Sim-{}".format(1+sim)
        ...
        for molecule in molecules:
            for atom in molecule:
                if atypes[atom]==C and mtypes[atom]<=N_PAOr:
                    boom.add([atypes[x] for x in molecule if mtypes[x]<=N_PAOr].count(C))
                    SR[time]+=1
                    break
...
mplt.mean_range_plot(dff, ax=ax, label='Actual Data',color='tab:blue')
ax.set_xlabel("Time (ps)")
ax.set_ylabel("# species generated from radicals")
